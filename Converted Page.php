...
/**
 * --------------------------------------------------------------------------
 * Game configuration — keyed by $gId passed from upstream context
 * Supports NM2 (New Mexico Roadrunner Cash) and LA5 (Louisiana Easy 5)
 * Both are Pick 5 games with main numbers 01-37, no bonus ball
 * --------------------------------------------------------------------------
 */
$leGameConfigs = [
    'NM2' => [
        ...
        'has_bonus'     => false,
        'bonus_label'   => '',
        'archive_route' => 'lottery-archives-pick5',
        'links' => [
            'skai'     => '/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=NM2',
            'ai_pred'  => '/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=NM2',
            'heatmap'  => '/picking-winning-numbers/artificial-intelligence/new-mexico-roadrunner-cash-heatmap-analysis',
            'skip_hit' => '/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=NM2',
            'mcmc'     => '/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=NM2',
        ],
    ],
    'LA5' => [
        'main_min'      => 1,
        ...
        'archive_route' => 'lottery-archives-pick5',
        'links' => [
            'skai'     => '/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=LA5',
            'ai_pred'  => '/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=LA5',
            'heatmap'  => '/picking-winning-numbers/artificial-intelligence/louisiana-easy-5-heatmap-analysis',
            'skip_hit' => '/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=LA5',
            'mcmc'     => '/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=LA5',
        ],
    ],
];

$leGid      = (isset($gId) && is_string($gId) && $gId !== '') ? trim((string) $gId) : 'NM2';
$leGc       = isset($leGameConfigs[$leGid]) ? $leGameConfigs[$leGid] : $leGameConfigs['NM2'];
$leMainMin  = (int) $leGc['main_min'];
$leMainMax  = (int) $leGc['main_max'];
$leHasBonus = (bool) $leGc['has_bonus'];
$leArchiveRoute = (string) $leGc['archive_route'];

$leStateName = isset($stateName) ? (string) $stateName : '';
$leGName     = isset($gName)     ? (string) $gName     : '';
$leStAbrev   = isset($stateAbrev) ? (string) $stateAbrev : '';

$leUrlSuffix = 'gId=' . rawurlencode($leGid)
    . '&stateName=' . rawurlencode($leStateName)
    . '&gName='     . rawurlencode($leGName)
    . '&sTn='       . rawurlencode(strtolower($leStAbrev));

$leArchiveUrl = '/' . $leArchiveRoute . '?' . $leUrlSuffix;
$leLowNumUrl  = '/lowest-drawn-number-analysis?' . $leUrlSuffix;
$leSkaiUrl    = (string) $leGc['links']['skai'];
$leAiPredUrl  = (string) $leGc['links']['ai_pred'];
$leHeatmapUrl = (string) $leGc['links']['heatmap'];
$leSkipHitUrl = (string) $leGc['links']['skip_hit'];
$leMcmcUrl    = (string) $leGc['links']['mcmc'];

$leMainMaxPad = ($leMainMax < 10) ? '0' . $leMainMax : (string) $leMainMax;
...
if ($leStateName !== '' && $leGName !== '') {
    $doc->setTitle('Results Intelligence — ' . $leStateName . ' • ' . $leGName . ' | LottoExpert.net');
}
 ...
 * --------------------------------------------------------------------------
 */
$latestRows = leFetchRecentDraws($db, (string) $dbCol, $leGid, 1);
...
$latestMainBalls = array_values(array_filter([$p1, $p2, $p3, $p4, $p5], static function ($b) {
    return $b !== '';
}));

$logo = leResolveLogo($leStAbrev, $leGName);

$rowsMain = leFetchRecentDraws($db, (string) $dbCol, $leGid, $nodCurrentMain);

[$mainCounts, $mainLastSeenIndex] = leInitRange($leMainMin, $leMainMax);
 ...
 * --------------------------------------------------------------------------
 */
$mainChartLabels   = leBuildNaturalLabels($leMainMin, $leMainMax);
$mainChartValues   = [];
$mainRecencyValues = [];
...
$window50  = leFetchRecentDraws($db, (string) $dbCol, $leGid, 50);
$window300 = leFetchRecentDraws($db, (string) $dbCol, $leGid, 300);

[$counts50,  ] = leInitRange($leMainMin, $leMainMax);
[$counts300, ] = leInitRange($leMainMin, $leMainMax);
...
if ($drawDate !== '') {
    foreach ($latestMainBalls as $ball) {
        $prevDate = leGetPreviousOccurrenceDate($db, (string) $dbCol, $leGid, $drawDate, $ball, false);
        $drawsAgo = leGetDrawingsSinceDate($db, (string) $dbCol, $leGid, $prevDate, $drawDate);
        $drawHistoryRows[] = [
            'label'    => lePad2($ball),
            'prevDate' => $prevDate,
            'drawsAgo' => $drawsAgo,
...
$jsonLdPage = [
    ...
    '@type'       => 'WebPage',
    'name'        => 'Results Intelligence — ' . $leStateName . ' ' . $leGName,
    'description' => 'Number frequency analysis, recency, and draw history for ' . $leStateName . ' ' . $leGName . ' lottery.',
    'url'         => $canonicalNoQuery,
];
$jsonLdDataset = [
    '@context'    => 'https://schema.org',
    '@type'       => 'Dataset',
    'name'        => $leStateName . ' ' . $leGName . ' Frequency Data',
    'description' => 'Historical draw frequency data for ' . $leStateName . ' ' . $leGName . ' numbers 01–' . $leMainMaxPad . '.',
    'url'         => $canonicalNoQuery,
];
?>
<script type="application/ld+json">
<?php echo json_encode($jsonLdPage,    JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?>
</script>
<script type="application/ld+json">
    ...
    <div class="skai-hero-inner">
      <div class="skai-hero-top">
        ...
        <div class="skai-logo" aria-hidden="<?php echo $logo['exists'] ? 'false' : 'true'; ?>">
          <?php if ($logo['exists'] && $logo['url'] !== '') : ?>
            <img
              ...
              alt="<?php echo htmlspecialchars($leStateName . ' ' . $leGName, ENT_QUOTES, 'UTF-8'); ?>"
              width="110"
              height="110"
              loading="lazy"
              decoding="async"
        ...
        <div class="skai-hero-copy">
          ...
          <h1 class="skai-title">
            <?php echo htmlspecialchars($leStateName, ENT_QUOTES, 'UTF-8'); ?>
            &ndash;
          ...
          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="<?php echo htmlspecialchars($leSkaiUrl, ENT_QUOTES, 'UTF-8'); ?>">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="<?php echo htmlspecialchars($leAiPredUrl, ENT_QUOTES, 'UTF-8'); ?>">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($leSkipHitUrl, ENT_QUOTES, 'UTF-8'); ?>">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($leMcmcUrl, ENT_QUOTES, 'UTF-8'); ?>">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="<?php echo htmlspecialchars($leHeatmapUrl, ENT_QUOTES, 'UTF-8'); ?>">Heatmap Analysis</a>
          </div>
        </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The left panel shows the complete main-number distribution across all values 01&ndash;<?php echo htmlspecialchars($leMainMaxPad, ENT_QUOTES, 'UTF-8'); ?>. The right panel shows the recency distance for each number, revealing how long each value has been absent from recent draws.
        </p>
      </div>
    </div>
      ...
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values 01&ndash;<?php echo htmlspecialchars($leMainMaxPad, ENT_QUOTES, 'UTF-8'); ?> across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
        ...
        <div class="skai-grid">
          <div class="skai-card">
            <div class="skai-card-head skai-card-head--ember">
              ...
              <span class="skai-card-sub">Distance since last appearance for each main number (draws ago)</span>
            </div>
            <div class="skai-card-body">
              <div class="skai-chart-shell">
                <div class="skai-chart-frame skai-chart-frame--tall">
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use the table for exact counts, recency, and personal tracking. Quick filters help narrow the view without losing full access to the complete dataset. The draw window can be adjusted to broaden or narrow the historical range.
        </p>
      </div>
    </div>

    <div class="skai-controls">
      <form name="fqsearch" method="post" action="/all-us-lotteries/results-analysis?st=<?php echo htmlspecialchars($leStAbrev, ENT_QUOTES, 'UTF-8'); ?>&amp;stn=<?php echo htmlspecialchars($leStateName, ENT_QUOTES, 'UTF-8'); ?>&amp;gm=<?php echo htmlspecialchars($leGName, ENT_QUOTES, 'UTF-8'); ?>#tables">
    ...
    <div class="skai-section-body">
      <div class="skai-card">
        <div class="skai-card-head skai-card-head--horizon">
          Main numbers table
          <span class="skai-card-sub">Exact counts and recency for values 01&ndash;<?php echo htmlspecialchars($leMainMaxPad, ENT_QUOTES, 'UTF-8'); ?></span>
        </div>
        ...
        <div class="skai-table-wrap">
          <table id="skai-main-table" class="skai-table" aria-label="Main numbers frequency table">
            ...
            <tbody>
              <?php for ($i = $leMainMin; $i <= $leMainMax; $i++) : ?>
                <?php
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($leSkaiUrl, ENT_QUOTES, 'UTF-8'); ?>">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($leAiPredUrl, ENT_QUOTES, 'UTF-8'); ?>">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="<?php echo htmlspecialchars($leSkipHitUrl, ENT_QUOTES, 'UTF-8'); ?>">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($leMcmcUrl, ENT_QUOTES, 'UTF-8'); ?>">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($leHeatmapUrl, ENT_QUOTES, 'UTF-8'); ?>">Heatmap Analysis</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($leArchiveUrl, ENT_QUOTES, 'UTF-8'); ?>">Lottery Archives</a>
        <a class="skai-utility-link" href="<?php echo htmlspecialchars($leLowNumUrl, ENT_QUOTES, 'UTF-8'); ?>">Lowest Number Analysis</a>
      </div>
    </div>
  </section>
